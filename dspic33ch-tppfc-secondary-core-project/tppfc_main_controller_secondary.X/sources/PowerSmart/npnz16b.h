 ...
 * *********************************************************************************
 * Generic library header for z-domain compensation filter Assembler functions
 * CGS Version: 3.0.11
 * CGS Date:    01/06/2022
 * ********************************************************************************/
 ...
 * of the form [MAJOR][MINOR][REVISION] => version 1.2.3.xxx would be shown as 1203.
...
enum NPNZ_STATUS_FLAGS_e
{
    ...
    NPNZ_STATUS_TARGET_SINGLE    = 0b0000000000000000,
    NPNZ_STATUS_TARGET_MIRROR    = 0b0000000000000100,
...
/***************************************************************************************************
 * @enum    NPNZ_STATUS_TARGET_MIRROR_e
 * @brief   Enumeration of control output port mirroring control bits
 **************************************************************************************************/
enum NPNZ_STATUS_TARGET_MIRROR_e{
    NPNZ_TARGET_SINGLE = 0b0,                     ///< Controller target ports are not mirrored, primary source is active output
    NPNZ_TARGET_MIRRORED  = 0b1                   ///< Controller target ports are mirrored, target and alternate target are simultaneous, active outputs
};                                                ///< NPNZ Target Port Swap Control bits
typedef enum NPNZ_STATUS_TARGET_MIRROR_e NPNZ_STATUS_TARGET_MIRROR_t; ///< NPNZ Target Port Mirroring Control bits
extern volatile enum NPNZ_STATUS_TARGET_MIRROR_e npnzEnumControlTargetMirroring; ///< List Object Control Target Mirroring
...
struct NPNZ_FLAGS_s
{
    ...
    volatile enum NPNZ_STATUS_SATURATION_e    flagSaturation; ///< List of control loop saturation status flags in the NPNZ16b status word
    volatile enum NPNZ_STATUS_TARGET_MIRROR_e flagTargetMirror; ///< List of control output mirroring status flags in the NPNZ16b status word
    volatile enum NPNZ_STATUS_AGC_ENABLE_e    flagAgcControl; ///< List of adaptive gain control control bits in the NPNZ16b status word
    volatile enum NPNZ_STATUS_TARGET_SWAP_e   flagTargetSwap; ///< List of control output swapping control bits in the NPNZ16b status word
    volatile enum NPNZ_STATUS_SOURCE_SWAP_e   flagSourceSwap; ///< List of control input swapping control bits in the NPNZ16b status word
    volatile enum NPNZ_STATUS_INPUT_INV_e     flagCtrlInputInversion; ///< List of control input inversion control bits in the NPNZ16b status word
    volatile enum NPNZ_STATUS_ENABLE_e        flagControlEnable; ///< List of control loop enable bits in the NPNZ16b status word
...
struct NPNZ_STATUS_s {
    ...
    struct {
        ...
        volatile bool mirror_target : 1;          ///< Bit 2: flag bit if control output is written to Target and AtlTarget simultaneously
...
struct NPNZ_EXTENSION_HOOKS_s{
    ...
    volatile uint16_t ExtHookStartFunctionParam;  ///< Parameter of function called (can be a variable or pointer to a data structure). This parameter is optional and needs to be supported by the Assembler routine to be pushed to the user function. Parameter support can be enabled/disabled for each hook function by selecting the respective option in PS-DCLD.
    ...
    volatile uint16_t ExtHookSourceFunctionParam; ///< Parameter of function called (can be a variable or a pointer to a data structure). This parameter is optional and needs to be supported by the Assembler routine to be pushed to the user function. Parameter support can be enabled/disabled for each hook function by selecting the respective option in PS-DCLD.
