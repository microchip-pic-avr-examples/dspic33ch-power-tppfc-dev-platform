...
{
    0x00004091, // Coefficient A1 will be multiplied with controller output u(n-1)
    0x0000126E  // Coefficient A2 will be multiplied with controller output u(n-2)
};

volatile int32_t ICOMP_BCoefficients [3] =
{
    0x00007FFF, // Coefficient B0 will be multiplied with error input e(n-0)
    0x00000264, // Coefficient B1 will be multiplied with error input e(n-1)
    0x00008265  // Coefficient B2 will be multiplied with error input e(n-2)
};
...
volatile int16_t ICOMP_post_shift_A = -1;         // Bit-shift value A used to perform control output value backward normalization
volatile int16_t ICOMP_post_shift_B = 0;          // Bit-shift value B used to perform control output value backward normalization
volatile fractional ICOMP_post_scaler = 0x62B6;   // Q15 fractional factor used to perform control output value backward normalization
